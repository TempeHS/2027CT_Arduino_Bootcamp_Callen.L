...
/*
  ...
  See image at IMG_0832.jpg (does not have line finder) or withLineFinder.jpg (with line finder as the name states in camelCase)
*/

const int BUTTON_PIN = 4;
const int LED_PIN = 6;
const int LINE_FINDER_PIN = 7;
...
void setup() {
  ...
  Serial.println("With the bonus of a line finder!");
  Serial.println("========================================");
...
void loop() {
  ...
  int lineState = digitalRead(LINE_FINDER_PIN);

  if (lineState != lastLineState) {

    if (lineState == LOW) {
      Serial.println();
      Serial.println("Oh, quite dark out there today ain't it?");
    } else {
      Serial.println();
      Serial.println("Oh damn, so bright!!!! Better get sunglasses");
    }
